...
- Routes (verified path strings; `/` and `/products` are contracted in
...
- Stable selectors:
  - **Contracted in repo:** `PRODUCTS_UI` in
    `cypress/configs/ui/products/products.ui.js` owns the listing region
    (`.features_items`), product card (`.product-image-wrapper`), product name
    (`.productinfo p`), and visible heading text (`All Products`).
  - The live application exposes no test attributes. Structural CSS selectors are therefore the
    verified fallback, centralized in config; visible text remains preferred for the heading.
  - Search/cart selector candidates remain out of scope for this requirement.
...
| ----------------------------------------------------- | ----- | ------------------------------- | ----------------------------------------------------------------------- |
| Approve draft `AE-PRODUCTS-002` → `active`?           | Owner | Yes for BUILD                   | **Resolved `2026-08-13`: promoted to `active`**                         |
